...
(function (global, factory) {
  ...
  jQuery.fn.extend({
    ...
    index: function index(elem) {
      ...
      return jQuery.inArray( // If it receives a jQuery原理 object, the first element is used
  ...
  /* Internal Use Only */
    ...
    thisCache = cache[id]; // jQuery原理 data() is stored in a separate object inside the object's internal data
  ...
  jQuery.fn.extend({
    data: function data(key, value) {
      var i,
          ...
          data,
          elem = this[0],
          attrs = elem && elem.attributes; // Special expections of .data basically thwart jQuery原理.access,
  ...
  jQuery.event = {
    ...
    trigger: function trigger(event, data, elem, onlyHandlers) {
      ...
      ontype = type.indexOf(":") < 0 && "on" + type; // Caller can pass in a jQuery原理.Event object, Object, or just an event type string

      event = event[jQuery.expando] ? event : new jQuery.Event(type, _typeof(event) === "object" && event); // Trigger bitmask: & 1 for native handlers; & 2 for jQuery原理 (always true)
      ...
      while ((cur = eventPath[i++]) && !event.isPropagationStopped()) {
        event.type = i > 1 ? bubbleType : special.bindType || type; // jQuery原理 handler
    ...
    dispatch: function dispatch(event) {
      ...
      var i,
          ...
          special = jQuery.event.special[event.type] || {}; // Use the fix-ed jQuery原理.Event rather than the (read-only) native event
          ...
          if (data) {
            if (data.events) {
              for (type in data.events) {
                if (special[type]) {
                  jQuery.event.remove(elem, type); // This is a shortcut to avoid jQuery原理.event.remove's overhead
                } else {
  ...
  jQuery.each({
    ...
    jQuery.fn[name] = function (selector) {
      ...
      for (; i <= last; i++) {
        ...
        jQuery(insert[i])[original](elems); // Modern browsers can apply jQuery原理 collections as arrays, but oldIE needs a .get()
  ...
  (function () {
    ...
    function computeStyleTests() {
      ...
      pixelMarginRightVal = reliableMarginRightVal = true; // Check for getComputedStyle so that this jQuery基础 is not run in IE<9.
  ...
  ajaxLocParts = rurl.exec(ajaxLocation.toLowerCase()) || []; // Base "constructor" for jQuery原理.ajaxPrefilter and jQuery原理.ajaxTransport
  ...
  jQuery.fn.andSelf = jQuery.fn.addBack; // Register as a named AMD module, since jQuery原理 can be concatenated with other
  ...
  var // Map over jQuery原理 in case of overwrite
